...
A Viewer can be displayed once coppafish has run through at least [call spots](overview#call-spots). Do this by
...
"Prob" is the Von-Mises probability method, "Anchor" is the anchor method (see [call spots](overview#call-spots)), and 
"OMP" is the Orthogonal Matching Pursuit method (see [OMP](overview#orthogonal-matching-pursuit)). The "OMP Score 
...
Images after all [filtering](overview#filter) are viewed by
