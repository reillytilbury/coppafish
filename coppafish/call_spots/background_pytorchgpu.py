...
def fit_background(
    ...
    background_vectors = torch.repeat_interleave(torch.eye(n_channels, device=device)[:, None, :], n_rounds, dim=1)
    ...
    background_weight = torch.ones((1, n_rounds, n_channels), device=device) * background_vectors[0, 0, 0] * weight_factor
    coef = torch.sum(spot_weight * background_weight, dim=1) / torch.sum(background_weight ** 2, dim=1)
    residual = spot_colors \
        - coef[:, None] * torch.ones((1, n_rounds, n_channels), device=device) * background_vectors[0, 0, 0]
