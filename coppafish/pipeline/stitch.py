...
def stitch(config: dict, nbp_basic: NotebookPage, local_yxz: np.ndarray, spot_no: np.ndarray) -> NotebookPage:
    ...
    with tqdm(total=2 * len(nbp_basic.use_tiles)) as pbar:
        ...
        for t in nbp_basic.use_tiles:
            ...
            t_neighb["north"] = np.where(
                np.sum(nbp_basic.tilepos_yx == nbp_basic.tilepos_yx[t, :] + [1, 0], axis=1) == 2
            )[0]
            t_neighb["east"] = np.where(
                np.sum(nbp_basic.tilepos_yx == nbp_basic.tilepos_yx[t, :] + [0, 1], axis=1) == 2
            )[0]
            for j in directions:
                ...
                if t_neighb[j].size > 0 and t_neighb[j] in nbp_basic.use_tiles:
                    shift, score, score_thresh = stich_shift.compute_shift(
                        ...
                        shifts[j]["z"],
                        config["shift_widen"],
                        config["shift_max_range"],
                        z_scale,
                        config["nz_collapse"],
                        config["shift_step"][2],
                    )[:3]
                    shift_info[j]["pairs"] = np.append(
                        shift_info[j]["pairs"], np.array([t, t_neighb[j][0]]).reshape(1, 2), axis=0
                    )
                    ...
                    shift_info[j]["score_thresh"] = np.append(
                        shift_info[j]["score_thresh"], np.array(score_thresh).reshape(1, 1), axis=0
                    )
                    ...
                    if np.sum(good_shifts) >= 3:
                        ...
                        for i in range(len(coords)):
                            shifts[j][coords[i]] = stich_shift.update_shifts(
                                shifts[j][coords[i]], shift_info[j]["shifts"][good_shifts, i]
                            )
                pbar.update(1)
    pbar.close()
    ...
    for j in directions:
        ...
        for i in np.where(good_shifts == False)[0]:
            ...
            shift_info[j]["shifts"][i], shift_info[j]["score"][i] = stich_shift.compute_shift(
                find_spots_base.spot_yxz(local_yxz, t, r, c, spot_no),
                find_spots_base.spot_yxz(local_yxz, t_neighb, r, c, spot_no),
                0,
                None,
                None,
                None,
    ...
    if all_tiles_connected:
        tile_origin = stitch_tile_origin.get_tile_origin(
            shift_info["north"]["pairs"],
            shift_info["north"]["shifts"],
            shift_info["east"]["pairs"],
            shift_info["east"]["shifts"],
            nbp_basic.n_tiles,
            centre_tile,
        )
    ...
    for j in directions:
        ...
        nbp_debug.__getattribute__(j + "_" + "final_shift_search")[:, 1] = [
            np.max(shifts[j][key]) for key in shifts[j].keys()
        ]
        nbp_debug.__getattribute__(j + "_" + "final_shift_search")[:, 2] = nbp_debug.__getattribute__(
            j + "_" + "start_shift_search"
        )[:, 2]
