...
def view_spot_brightness_hists(nb: Notebook):
    ...
    spot_colours_no_bg = nb.ref_spots.colours[:, :, use_channels] - np.repeat(
        nb.ref_spots.background_strength[:, np.newaxis, :], n_rounds, axis=1
    )
    ...
    for r in range(n_rounds):
        for c in range(n_channels):
            ...
            axes[r, c].hist(
                spot_brightness[r][c],
                bins=np.linspace(min_brightness[c], max_brightness[c], 100),
...
class GESpotViewer:
    ...
    def load_spots(self, gene_index: int, use_ge=True):
        ...
        color_norm = np.repeat(
            np.mean(nb.call_spots.color_norm_factor, axis=0)[
...
class ViewBleedCalc:
    def __init__(self, nb: Notebook):
        ...
        self.isolated_spots = (
            nb.ref_spots.colours[nb.ref_spots.isolated][:, :, nb.basic_info.use_channels] - background_strength
        )
        ...
        if nb.file_names.initial_bleed_matrix is not None:
            ...
            self.default_bleed = np.load(
                importlib_resources.files("coppafish.setup").joinpath("default_bleed.npy")
            ).copy()
        ...
        self.colour_vectors = self.isolated_spots.reshape(
            (self.isolated_spots.shape[0] * self.nb.basic_info.n_rounds, len(self.nb.basic_info.use_channels))
        )
...
class GeneScoreScatter:
    ...
    def add_gene_slider(self):
        ...
        self.gene_slider = Slider(
            self.gene_slider_ax, "Gene", 0, self.n_genes - 1, valinit=self.gene_no, valstep=1, orientation="vertical"
        )
        ...
        self.gene_text = self.gene_text_ax.text(
            0.5,
            0.5,
            self.nb.call_spots.gene_names[self.gene_no],
    ...
    def plot_scatter(self, event=None):
        ...
        for row in range(7):
            for col in range(7):
                ...
                self.ax[row][col].set_ylim(
                    np.percentile(spot_round_brightness, 1), np.percentile(spot_round_brightness, 99)
                )
                self.ax[row][col].set_xlim(
                    np.percentile(spot_round_brightness, 1), np.percentile(spot_round_brightness, 99)
                )
                ...
                self.ax[row][col].plot(
                    ...
                    m * x + c,
                    color="red",
                    label="R$^2$ = " + str(round(r_squared[0, 1], 2)) + "\n" + "Slope = " + str(round(m, 2)),
                )
...
class GeneProbs:
    ...
    def plot_gene_probs(self, event=None):
        ...
        for i in range(self.n_genes):
            self.ax.axhline(
                np.sum(num_assignments[: i + 1]) - 0.5,
                color="w",
                linewidth=1,
                linestyle="--",
    ...
    def add_rectangle(self, index):
        ...
        for rectangle in self.ax.patches:
            rectangle.remove()
        # We can then add a new rectangle to the plot
        self.ax.add_patch(Rectangle((-0.5, index - 0.5), self.n_genes, 1, fill=False, edgecolor="white"))

    def add_widgets(self):
        # Initialise buttons and cursors
        # 1. We would like each row of the plot to be clickable, so that we can view the observed spot.
        mplcursors.cursor(self.ax, hover=False).connect("add", lambda sel: self.row_clicked(sel))
        # 2. We would like to add a white rectangle around the observed spot when we hover over it
        mplcursors.cursor(self.ax, hover=2).connect("add", lambda sel: self.add_rectangle(sel.index[0]))
