...
class view_background:
    ...
    def __init__(
        self, nb: Notebook, spot_no: int, method: str = "omp", check: bool = True, track_info: Optional[List] = None
    ):
        """
        ...
        self.ax[self.coef_plot_ind[0]].get_shared_x_axes().join(
            self.ax[self.coef_plot_ind[0]], self.ax[self.coef_plot_ind[1]]
        )
        ...
        self.text_box = TextBox(
            text_ax,
            r"background_shift, $\lambda_b$",
            self.background_weight_shift,
            color="k",
    ...
    def update_data(self):
        ...
        if self.check:
            if np.abs(coef_weight - self.track_info["background_coefs"]).max() > self.check_tol:
                raise ValueError(
                    "Background calculated is not the same as that from get_track_info\n"
    ...
    def set_up_plots(self):
        for i in range(len(self.ax)):
            if i == self.weight_plot_ind:
                ...
                self.im[i] = self.ax[i].imshow(
                    self.im_data[i], cmap="bwr", vmin=-self.vmax[i], vmax=self.vmax[i], aspect=aspect
                )
            if np.isin(i, self.coef_plot_ind):
        ...
        # Weight
        self.fig.colorbar(
            self.im[self.weight_plot_ind], ax=self.ax[self.weight_plot_ind], fraction=0.02, pad=0.04, aspect=51
        )
        # Dot product
        dp_inds = np.setdiff1d(np.arange(len(self.im_data)), self.spot_color_plot_inds + [self.weight_plot_ind])
        self.fig.colorbar(self.im[dp_inds[2]], ax=self.ax[dp_inds[2]], fraction=0.046, pad=0.04, aspect=51)
        # Dot product weight
        self.weight_dp_cbar = self.fig.colorbar(
            self.im[dp_inds[3]], ax=self.ax[dp_inds[3]], fraction=0.046, pad=0.04, aspect=51
        )
        if self.vmax[dp_inds[3]] > 2.5:
            ...
            spacing = 0.1
        self.weight_dp_cbar.set_ticks(
            np.arange(
                -round_any(self.vmax[dp_inds[3]], spacing, "floor"),
                round_any(self.vmax[dp_inds[3]], spacing, "floor") + spacing / 2,
                spacing,
            )
        )
    ...
    def set_titles(self):
        self.title = [
            ...
            r"Background Codes, $\mathbf{B} = \sum_{C="
            + str(0)
            + "}^{"
            + str(self.n_channels_use - 1)
            + "}\mathbf{B}_C$",
            ...
            r"Background, $\mathbf{B_s}=\sum_{C="
            + str(0)
            + "}^{"
            + str(self.n_channels_use - 1)
            + "}\mu_{sC}\mathbf{B}_C$",
        ...
        plt.suptitle(
            f"Background Coefficient Calculation for for spot {self.spot_no}",
    ...
    def update(self):
        ...
        if np.abs(max_weight_dp - self.weight_dp_cbar.vmax) > self.change_weight_dp_cbar_tol:
            ...
            self.weight_dp_cbar.set_ticks(
                np.arange(
                    -round_any(self.weight_dp_cbar.vmax, spacing, "floor"),
