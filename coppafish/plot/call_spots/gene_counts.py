...
class gene_counts:
    ...
    def __init__(
        self,
        nb: Notebook,
        fake_bled_codes: Optional[np.ndarray] = None,
        ...
        if nb.has_page("omp"):
            ...
            self.score += [omp_spot_score(self.nbp_omp).astype(np.float16)]
        ...
        for i in range(self.n_plots):
            (self.plots[i],) = self.ax.plot(
                np.arange(self.n_genes),
                np.histogram(self.gene_no[i][self.use[i]], np.arange(self.n_genes + 1) - 0.5)[0],
        ...
        plt.axis("off")
        self.button_labels = ["Ref Spots", "Ref Spots - Fake Genes"]
    ...
    def update_use(self):
        ...
        self.use = [
            np.array([self.score[i] > self.score_thresh[i], self.intensity[i] > self.intensity_thresh]).all(axis=0)
            for i in range(self.n_plots)
        ]
