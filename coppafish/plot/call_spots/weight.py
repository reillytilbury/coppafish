...
class view_weight:
    ...
    def get_cax_lim(self):
        ...
        weights = [
            val / np.sum(val) * self.n_rounds_use * self.n_channels_use for val in self.track_info["inverse_var"][2:]
        ]
    ...
    def set_up_plots(self):
        ...
        self.fig.colorbar(
            self.im[self.n_plots_top_row], ax=self.ax[self.n_plots_top_row :], fraction=0.046, pad=0.01, aspect=40
        )
    ...
    def set_titles(self):
        self.title = [
            ...
            r"$\omega^2_{{si}_{rc}} = n_{r}n_{c}" r"\frac{\sigma^{-2}_{{si}_{rc}}}{\sum_{rc}\sigma^{-2}_{{si}_{rc}}}$",
            r"Background, $\mathbf{B_s}=\sum_{g="
            + str(self.n_genes)
            + "}^{"
            + str(self.n_genes_all - 1)
            + "}\mu_{sg}\mathbf{b}_g$",
        ]
        for i in range(self.n_iter - 1):
            g = self.track_info["gene_added"][i + 2]
        ...
        plt.suptitle(
            r"Weight Squared, $\mathbf{\omega}_{si}$, Calculation, at iteration "
    ...
    def show_background(self, event):
        ...
        if y_click < self.ax[0].bbox.extents[1] and x_click < self.ax[1].bbox.extents[0]:
            view_background(self.nb, self.spot_no, self.method, track_info=[self.track_info, self.vmax[0]])
