...
class view_score:
    ...
    def __init__(
        ...
        g: Optional[int] = None,
        iter: int = 0,
        omp_fit_info: Optional[List] = None,
        ...
        if iter >= self.n_iter or iter < 0:
            warnings.warn(
                f"Only {self.n_iter} iterations for this pixel but iter={iter}, " f"setting iter = {self.n_iter - 1}."
            )
            iter = self.n_iter - 1
        self.iter = iter
        if g is None:
        ...
        for i in range(len(text_box_labels)):
            text_ax = self.fig.add_axes(
                [self.subplot_adjust[1] + 0.05, self.subplot_adjust[3] - 0.15 * (i + 1), 0.05, 0.04]
            )
    ...
    def update_data(self):
        ...
        background_var = (
            self.track_info["background_coefs"] ** 2
            @ self.bled_codes[-self.n_channels_use :].reshape(self.n_channels_use, -1) ** 2
            * self.alpha
            + self.beta**2
        )
        gene_var = (
            self.track_info["coef"][self.iter + 1, : self.n_genes] ** 2
            @ self.bled_codes[: self.n_genes].reshape(self.n_genes, -1) ** 2
            * self.alpha
        )  # will be 0 if iter=0
        ...
        if self.check_weight:
            ...
            if np.abs(weight - self.track_info["inverse_var"][2 + self.iter]).max() > self.check_tol:
                raise ValueError(
                    "Weight calculated is not the same as that from get_track_info\n"
        ...
        if self.check_weight and self.g == self.track_info["gene_added"][2 + self.iter] and self.g < self.n_genes:
            ...
            if np.abs(dot_product_weight.sum() - self.track_info["dot_product"][2 + self.iter]) > self.check_tol:
                raise ValueError(
                    "dot_product_weight calculated is not the same as that from get_track_info.\n"
                    "Set check_weight=False to skip this error."
                )
    ...
    def set_up_plots(self):
        for i in range(len(self.ax)):
            if i == 4:
                self.im[i] = self.ax[i].imshow(self.im_data[i], cmap="viridis", vmin=0, vmax=self.vmax[i])
            else:
                self.im[i] = self.ax[i].imshow(self.im_data[i], cmap="bwr", vmin=-self.vmax[i], vmax=self.vmax[i])
        ...
        cbar.set_ticks(
            np.arange(
                -round_any(self.vmax[0], spacing, "floor"),
                round_any(self.vmax[0], spacing, "floor") + spacing / 2,
                spacing,
            )
        )
        cbar = self.fig.colorbar(self.im[2], ax=self.ax[2:4], fraction=0.046, pad=0.04, aspect=51)
        cbar.set_ticks(
            np.arange(-round_any(self.vmax[2], 0.2, "floor"), round_any(self.vmax[2], 0.2, "floor") + 0.1, 0.2)
        )
        ...
        cbar.set_ticks(
            np.arange(-round_any(self.vmax[5], 0.1, "floor"), round_any(self.vmax[5], 0.1, "floor") + 0.05, 0.1)
        )
    ...
    def set_titles(self):
        self.title = [
            r"Spot Color, $\mathbf{\zeta_s}$",
            f"Iter {self.iter} Residual, " + r"$\mathbf{\zeta_{si}}$",
            ...
            f"Dot Product:"
            + r" $\sum_{rc}\delta_{{sig}_{rc}}$"
            + f"= {np.around(self.dp_val, 2)}\n"
            + r"$\delta_{{sig}_{rc}} = \tilde{\zeta}_{{si}_{rc}}b_{g_{rc}}$",
            "Score:"
            + r" $\sum_{rc}\Delta_{{sig}_{rc}}$"
            + f"= {np.around(self.dp_weight_val, 2)}\n"
            + r"$\Delta_{{sig}_{rc}} = \omega^2_{{si}_{rc}}\delta_{{sig}_{rc}}$",
        ]
        for i in range(len(self.ax)):
            if i == 3:
                is_fail_thresh = self.g >= self.n_genes or np.isin(
                    self.g, self.track_info["gene_added"][2 : self.iter + 2]
                )
            elif i == 5:
        ...
        if self.g == self.track_info["gene_added"][2 + self.iter]:
            ...
            title_extra = " "
        plt.suptitle(
            f"Dot Product Calculation for{title_extra}Gene {self.g}, {self.gene_names[self.g]}, on iteration "
