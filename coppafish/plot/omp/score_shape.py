...
class view_omp_score:
    ...
    def __init__(
        ...
        for i in range(self.nz):
            self.im[i] = self.ax[i].imshow(
                expected_image_plot[:, :, i],
                vmin=-self.vmax_expected,
                vmax=self.vmax_expected,
                cmap="bwr",
                extent=self.plot_extent,
            )
            self.im[i + self.nz] = self.ax[i + self.nz].imshow(
                self.coef_images[self.gene_no, :, :, i],
                vmin=-self.vmax_coef,
                vmax=self.vmax_coef,
                cmap="bwr",
                extent=self.plot_extent,
        ...
        cbar_ax = self.fig.add_axes(
            [
                self.subplot_adjust[1] + 0.01,
                mid_point + gap_size / 2,
                0.005,
                self.subplot_adjust[3] - mid_point - gap_size / 2,
            ]
        ...
        cbar_ax = self.fig.add_axes(
            [
                self.subplot_adjust[1] + 0.01,
                self.subplot_adjust[2] + gap_size / 5,
                0.005,
                mid_point - self.subplot_adjust[2] - gap_size / 2,
            ]
        )  # left, bottom, width, height
        ...
        plt.suptitle(
            f"OMP Score Calculation for Spot {spot_no}, Gene {self.gene_no}: {self.gene_names[self.gene_no]}",
            x=(self.subplot_adjust[0] + self.subplot_adjust[1]) / 2,
        )

        # Add text box to change score multiplier
        text_ax = self.fig.add_axes([self.subplot_adjust[1] + 0.062, self.subplot_adjust[2] + gap_size / 5, 0.05, 0.04])
        self.text_box = TextBox(
            text_ax,
            "Score\n" + r"Multiplier, $\rho$",
            str(np.around(self.score_multiplier, 2)),
            color="k",
    ...
    def get_score(self, expected_image):
        ...
        score = omp_spot_score(self.nbp_omp)
    ...
    def expected_image(self):
        ...
        max_score = self.score_multiplier * np.sum(self.expected_sign_image == 1) + np.sum(
            self.expected_sign_image == -1
        )
    ...
    def set_coef_plot_title(self):
        ...
        self.ax[mid_z + self.nz].set_title(
            f"Coefficient for Gene {self.gene_no}: {self.gene_names[self.gene_no]}, "
            f"Score, {score_str}, = {str(np.around(self.score, 2))}",
            color=color,
        )
        ...
        for i in range(self.nz):
            ...
            for j in range(len(pos_yx[0])):
                ...
                rectangle = plt.Rectangle(
                    (pos_yx[1][j] + self.plot_extent[0], self.plot_extent[3] - pos_yx[0][j] - 1),
                    1,
                    1,
                    fill=False,
                    ec="lime",
            ...
            for j in range(len(neg_yx[0])):
                rectangle = plt.Rectangle(
                    (neg_yx[1][j] + self.plot_extent[0], self.plot_extent[3] - neg_yx[0][j] - 1),
                    1,
                    1,
                    fill=False,
                    ec="lime",
