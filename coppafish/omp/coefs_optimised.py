...
def fit_coefs_single(
    bled_codes: jnp.ndarray, pixel_color: jnp.ndarray, genes: jnp.ndarray
...
def fit_coefs(
    bled_codes: jnp.ndarray, pixel_colors: jnp.ndarray, genes: jnp.ndarray
) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def fit_coefs_weight_single(
    bled_codes: jnp.ndarray, pixel_color: jnp.ndarray, genes: jnp.ndarray, weight: jnp.ndarray
) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def fit_coefs_weight(
    bled_codes: jnp.ndarray, pixel_colors: jnp.ndarray, genes: jnp.ndarray, weight: jnp.ndarray
) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def get_best_gene_base(
    ...
    all_scores = dot_product_optimised.dot_product_score(
        residual_pixel_color[None], all_bled_codes, norm_shift, inverse_var[None]
    )[0]
...
def get_best_gene_first_iter_single(
    residual_pixel_color: jnp.ndarray,
    all_bled_codes: jnp.ndarray,
    ...
    best_gene, pass_score_thresh = get_best_gene_base(
        residual_pixel_color, all_bled_codes, norm_shift, score_thresh, 1 / background_var, background_genes
    )
...
def get_best_gene_first_iter(
    ...
    vmap = jax.vmap(
        get_best_gene_first_iter_single, in_axes=(0, None, 0, None, None, None, None, None), out_axes=(0, 0, 0)
    )
    best_genes, pass_score_thresholds, backgrounds_vars = jax.pmap(
        vmap, in_axes=(0, None, 0, None, None, None, None, None), out_axes=(0, 0, 0)
    )(
        residual_pixel_colors_batched,
        all_bled_codes,
        background_coefs_batched,
        norm_shift,
        score_thresh,
        alpha,
        beta,
    ...
    # Combine batches
    return (
        best_genes.reshape((n_pixels + padding))[:n_pixels],
...
def get_best_gene_single(
    ...
    best_gene, pass_score_thresh = get_best_gene_base(
        ...
        score_thresh,
        inverse_var,
        jnp.append(background_genes, genes_added),
    )
    ...
    with tqdm(total=max_genes, disable=no_verbose) as pbar:
        ...
        for i in range(max_genes):
            ...
            if i == 0:
                ...
                gene_coefs[
                    np.asarray(continue_pixels[fail_score_thresh])[:, np.newaxis],
                    np.asarray(added_genes[fail_score_thresh]),
                ] = np.asarray(i_coefs[fail_score_thresh])
            ...
            if i == 0:
                ...
                added_genes = jnp.hstack(
                    (added_genes[pass_score_thresh], i_added_genes[pass_score_thresh, jnp.newaxis])
                )
            ...
            if weight_coef_fit:
                residual_pixel_colors, i_coefs = fit_coefs_weight(
                    bled_codes, pixel_colors, added_genes, jnp.sqrt(inverse_var)
                )
            else:
            ...
            if i == max_genes - 1:
                ...
                gene_coefs[jnp.asarray(continue_pixels)[:, jnp.newaxis], np.asarray(added_genes)] = jnp.asarray(i_coefs)
...
def get_pixel_coefs_yxz(
    ...
    z_chunk_size: int,
    n_genes: int,
    transform: Union[np.ndarray, np.ndarray],
    color_norm_factor: Union[np.ndarray, np.ndarray],
    initial_intensity_thresh: float,
    bled_codes: Union[np.ndarray, np.ndarray],
